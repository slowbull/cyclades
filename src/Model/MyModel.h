...
class MyModel : public Model {
 ...
 public:
    MyModel(const std::string &input_line) {
    ...
    double ComputeLoss(const std::vector<Datapoint *> &datapoints) override {
		...
		for (int i = 0; i < datapoints.size(); i++) {
			...
			loss += pow((cross_product - datapoint->GetLabel()), 2);
		}
	...
	//int row = ((MyDatapoint *)datapoint)->row;
	double cp = 0;
	...
	double partial_grad = 2 * (cp - datapoint->GetLabel());
	for (int i = 0; i < datapoint->GetCoordinates().size(); i++) {
	    int index = datapoint->GetCoordinates()[i];
	    double weight = datapoint->GetWeights()[i];
